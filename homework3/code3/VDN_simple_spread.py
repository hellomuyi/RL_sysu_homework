...
def parse():
    ...
    parser.add_argument("--exp_name", default='VDN', type=str)
    parser.add_argument("--log_dir", default='./result/VDN/', type=str)
    parser.add_argument("--save_dir", default='./result/VDN/', type=str)  # 学习曲线对应的数据
    parser.add_argument("--print_rate", default=100, type=int)  # 每隔save_rate个episodes打印日志
    parser.add_argument("--checkpoint_dir", default='.result/VDN/', type=str)
...
class ReplayBuffer:
    def __init__(self, buffer_size, episode_len, obs_dim, act_dim, num_agents):
        """
        VDN用到RNN，其隐藏状态的时序性质使得训练必须得以回合为单位
...
class VDNNet(nn.Module):
    ...
    def forward(self, q_values):
        return t.sum(q_values, dim=1, keepdim=True)
...
class VDNAgent:
    def __init__(self, env, arglist):
        ...
        self.hidden_size = arglist.hidden_size
    ...
    def update(self):
        ...
        eval_hidden = t.zeros((self.batch_size, self.num_agents, self.hidden_size)).to(self.device)
        target_hidden = t.zeros((self.batch_size, self.num_agents, self.hidden_size)).to(self.device)
        last_act_n = t.zeros((self.batch_size, self.num_agents, self.act_dim), dtype=t.int64).to(self.device)
        # VDN用到RNN，其隐藏状态的时序性质使得训练必须得以episode为单位
        losses = []
        for idx_step in range(self.max_episode_len):
            ...
            # 计算Q值
            q_eval, eval_hidden = self.eval_drqn_net(obs_n, last_act_n, eval_hidden)  # 输出(bs, num_agents, num_act) (bs, num_agents, hidden_size)
            ...
            # 基于值分解,计算团队Q值
            q_total_eval = self.eval_vdn_net(q_eval)  # t.sum(q_eval, -1, keepsim=True)  # (bs, 1)

            # 计算目标Q值
            target_hidden_ = target_hidden.clone().detach()
            q_next, target_hidden = self.target_drqn_net(new_obs_n, act_n, target_hidden)  # 输出(bs, num_agents, num_act) (bs, num_agents, hidden_size)
...
def play_one_game(agent, env):
    # 初始化eval_hidden
    eval_hidden_n = t.zeros((agent.num_agents, agent.hidden_size)).to(agent.device)
    # 初始化lastaction
