...
def parse():
    ...
    parser.add_argument("--exp_name", default='QMIX', type=str)
    parser.add_argument("--log_dir", default='./result/QMIX/', type=str)
    parser.add_argument("--save_dir", default='./result/QMIX/', type=str)  # 学习曲线对应的数据
    parser.add_argument("--print_rate", default=100, type=int)  # 每隔save_rate个episodes打印日志
    parser.add_argument("--checkpoint_dir", default='.result/QMIX/', type=str)
...
class QMIXNet(nn.Module):
    def __init__(self, obs_n_dim, hidden_size, num_agents, is_two_hyper_layers):
        ...
        self.hyper_w1 = init_(nn.Linear(obs_n_dim, num_agents))

        self.hyper_b1 = init_(nn.Linear(obs_n_dim, 1))
...
class QMIXAgent:
    def __init__(self, env, arglist):
        ...
        params_qmix = (self.obs_dim * self.num_agents, arglist.hidden_size_qmix, self.num_agents, False)
    ...
    def update(self):
        ...
        for idx_step in range(self.max_episode_len):
            ...
            q_total_eval = self.eval_qmix_net(q_eval, obs_n.view(self.batch_size, self.obs_dim*self.num_agents))
            # 计算目标Q值
            ...
            # 基于值分解,计算目标团队Q值
            q_total_next = self.target_qmix_net(q_next, new_obs_n.view(self.batch_size, self.obs_dim*self.num_agents))
