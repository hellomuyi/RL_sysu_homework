...
def parse():
    ...
    parser.add_argument("--exp_name", default='QMIX2', type=str)
    parser.add_argument("--log_dir", default='./result/QMIX2/', type=str)
    parser.add_argument("--save_dir", default='./result/QMIX2/', type=str)  # 学习曲线对应的数据
    parser.add_argument("--print_rate", default=100, type=int)  # 每隔save_rate个episodes打印日志
    parser.add_argument("--checkpoint_dir", default='.result/QMIX2/', type=str)
...
class QMIXAgent:
    def __init__(self, env, arglist):
        ...
        params_qmix = ((self.act_dim+self.obs_dim) * self.num_agents, arglist.hidden_size_qmix, self.num_agents, False)
    ...
    def update(self):
        ...
        for idx_step in range(self.max_episode_len):
            ...
            states = t.cat((obs_n, last_act_n), dim=-1)  
            q_total_eval = self.eval_qmix_net(q_eval, 
                                              states.view(self.batch_size, (self.obs_dim+self.act_dim)*self.num_agents))
            # 计算目标Q值
            ...
            # 基于值分解,计算目标团队Q值
            states_next = t.cat((new_obs_n, act_n), dim=-1) 
