...
def parse():
    parser = argparse.ArgumentParser(description="TD3 for LunarLanderContinuous-v2")
    ...
    parser.add_argument('--buffer_size', default=int(1e5), type=int, help='')
    parser.add_argument('--warm_up', default=1000, type=int, help='')
    ...
    parser.add_argument('--noise_clip', default=0.5, type=float, help='')  # 策略噪声的clip
    parser.add_argument('--tau', default=1e-2, type=float, help='')

    parser.add_argument("--exp_name", default='TD3_LunarLanderContinuous-v2', type=str)
    parser.add_argument("--log_dir", default='./result/TD3_LunarLanderContinuous-v2/', type=str)
    parser.add_argument("--save_dir", default='./result/TD3_LunarLanderContinuous-v2/', type=str)  # 学习曲线对应的数据
    parser.add_argument("--checkpoint_dir", default='.result/TD3_LunarLanderContinuous-v2/', type=str)
...
class Agent:
    def __init__(self, env, arglist):
        ...
        self.replay_buffer = ReplayBuffer(buffer_size=arglist.buffer_size, state_dim=self.state_dim, action_dim=self.action_dim)
    ...
    def update(self):
        ...
        target_noise = t.clamp(self.policy_noise*t.randn(actions.size()), min=-self.noise_clip, max=self.noise_clip) * self.action_max  # noise clip
        target_actions = self.target_actor.forward(new_states) + target_noise.to(self.device)
        ...
        if self.critic_update_counter % self.update_interval == 0:
            ...
            actor_loss = self.critic_1.forward(states, self.actor(states))
...
def main(arglist, training=True):
    ...
    if training:
        ...
        file_name = arglist.save_dir + 'TD3.pkl'
        ...
        env.seed(1)
        agent.critic_1.load_state_dict(t.load(arglist.save_dir+'critic_1'))
        agent.critic_2.load_state_dict(t.load(arglist.save_dir+'critic_2'))
        agent.actor.load_state_dict(t.load(arglist.save_dir+'actor'))
        agent.target_critic_1.load_state_dict(t.load(arglist.save_dir+'target_critic_1'))
        agent.target_critic_2.load_state_dict(t.load(arglist.save_dir+'target_critic_2'))
        agent.target_actor.load_state_dict(t.load(arglist.save_dir+'target_actor'))
